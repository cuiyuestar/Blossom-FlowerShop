...
public class ActivityOngoingChainFilter implements ActivityParticipateChainFilter<Activity> {

    /**
     * 处理逻辑
     * @param requestParam
     */
    public void handler(Activity requestParam) {
        if(requestParam==null||requestParam.getStatus()==0){
            throw new ActivityException(MessageConstant.ACTIVITY_ISEXIST);
        }
    }
